...
export const RecentPullRequests: React.FC<RecentPullRequestsProps> = ({
  pullRequests,
}) => {
  if (!pullRequests || pullRequests.length === 0) {
    return (
      <EmptyState
        title="No Pull Requests generated"
        description="AI generated documentation updates will appear here as PRs."
      />
    );
  }
        ...
        return (
          ...
          >
            <div className="flex items-start gap-3.5 min-w-0">
              <span
                className={`text-lg shrink-0 ${isMerged ? "text-purple-500 font-black" : "text-emerald-500"}`}
              >
                {isMerged ? "🟣" : "🟢"}
              </span>
