...
export const RepositoryTable: React.FC<RepositoryTableProps> = ({
  ...
  if (!repositories || repositories.length === 0) {
    return (
      <EmptyState
        ...
        description="Install the DocPulse GitHub App to sync your repositories."
      />
    );
  }
  ...
  const handleDelete = async (id: string) => {
    const ok = window.confirm(
      "Are you sure you want to disconnect this repository and clear all execution history?",
    );
    if (!ok) return;
            ...
            {repositories.map((repo) => {
              ...
              return (
                ...
                >
                  ...
                  <td className="px-6 py-4">
                    <span
                      className={`inline-flex items-center gap-1.5 text-xs font-semibold ${isActive ? "text-emerald-600 dark:text-emerald-400" : "text-zinc-400"}`}
                    >
                      <span
                        className={`w-1.5 h-1.5 rounded-full ${isActive ? "bg-emerald-500 animate-pulse" : "bg-zinc-400"}`}
                      />
                  ...
                  <td className="px-6 py-4">
                    <WorkflowStatusBadge
                      status={repo.latestWorkflow || "completed"}
                    />
                  </td>
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-3 text-xs">
                      ...
                      <button
                        ...
                        title={
                          isActive
                            ? "Must deactivate first to disconnect"
                            : "Delete local record"
                        }
                      >
                        Disconnect
