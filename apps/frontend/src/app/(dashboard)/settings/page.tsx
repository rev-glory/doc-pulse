...
export default function SystemSettingsPage(): React.JSX.Element {
  const [activeTab, setActiveTab] = useState<"profile" | "system" | "connect">(
    "profile",
  );
  ...
  const { data: installations, isLoading: isInstLoading } = useApiQuery({
  ...
  const handleConnectRepository = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setConnectSuccess(
        `✓ Repository "${result.fullName}" connected successfully!`,
      );
      setConnectOwner("");
      setConnectRepoName("");
    } catch (err: any) {
  ...
  if (sysError || profileError || prefsError) {
    const errorMsg =
      ...
      prefsError?.message ||
      "Failed to retrieve settings parameters.";
    return (
      <div>
        <PageHeader title="System Settings" />
        <ErrorState
    ...
    <div className="space-y-8 animate-fade-in">
      ...
      {activeTab === "profile" && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-5xl items-start">
          ...
          <SectionCard
            title="User Profile"
            description="Your personal identity on DocPulse."
          >
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-xs text-zinc-500">
                      Profile photo synced from GitHub
                    </span>
                  </div>
                </div>
              )}
          ...
          <SectionCard
            title="Notification & Theme Preferences"
            description="Manage email alerts, visual styles, and engine models."
          >
            <form onSubmit={handleUpdatePrefs} className="space-y-5 text-xs">
              ...
              <div className="space-y-1.5">
                ...
                <span className="block text-[10px] text-zinc-400">
                  Google Gemini API is the active language model. Exposing
                  alternative AI nodes is currently locked.
                </span>
              </div>
              ...
              <div className="flex items-center gap-3 py-2 border-t border-b border-zinc-100 dark:border-zinc-850">
                ...
                <div>
                  <label
                    htmlFor="emailAlerts"
                    className="font-bold text-zinc-800 dark:text-zinc-200 block cursor-pointer"
                  >
                    Enable email notification alerts
                  </label>
                  <span className="block text-[10px] text-zinc-400">
                    Receive diagnostic messages when human reviews require
                    attention.
                  </span>
                </div>
              </div>
              ...
              <div className="p-3 bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg text-xs space-y-1">
                <div className="flex justify-between">
                  ...
                  <span className="font-mono font-bold text-zinc-700 dark:text-zinc-300 uppercase">
                ...
                <div className="flex justify-between">
                  ...
                  <span
                    className={`font-bold ${userPrefs?.notifications?.email ? "text-emerald-600" : "text-zinc-400"}`}
                  >
      ...
      {activeTab === "connect" && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-5xl items-start">
          <SectionCard
            ...
            description="Manually connect a specific GitHub repository to DocPulse using POST /repositories/connect."
          >
            <form
                  ...
                  >
                    ...
                    {(installations || []).map((inst) => (
                      <option key={inst.id} value={inst.id}>
                        {inst.accountLogin} ({inst.accountType}) — ID:{" "}
                        {inst.installationId}
                      </option>
                    ))}
                  </select>
          ...
          >
            <div className="space-y-4 text-xs text-zinc-600 dark:text-zinc-400 leading-relaxed">
              <p>
                ...
                on the Repositories page) fetches all repos accessible to a
                GitHub App installation in bulk.
              </p>
              <p>
                Use this form to connect a{" "}
                <strong className="text-zinc-800 dark:text-zinc-200">
                  single specific repository
                </strong>{" "}
                by name, useful when:
              </p>
      ...
      {activeTab === "system" && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-5xl">
          ...
          <SectionCard
            ...
            description="Defaults for user session and interface."
          >
          ...
          <SectionCard
            ...
            description="LangGraph pipeline trigger points."
          >
            <div className="space-y-4 text-xs">
              <div className="flex justify-between items-center py-2.5 border-b border-zinc-150 dark:border-zinc-800">
                <span className="font-semibold text-zinc-500 dark:text-zinc-400">
              ...
              <div className="py-2.5">
                <span className="block font-semibold text-zinc-500 dark:text-zinc-400 mb-1.5">
                  Webhook Target URL
                </span>
          ...
          <SectionCard
            ...
            description="Background consumer limits."
          >
            <div className="space-y-4 text-xs">
              <div className="flex justify-between items-center py-2.5 border-b border-zinc-150 dark:border-zinc-800">
                <span className="font-semibold text-zinc-500 dark:text-zinc-400">
                  BullMQ Concurrency Bound
                </span>
                <span className="font-mono bg-zinc-100 dark:bg-zinc-800 px-2 py-0.5 rounded text-zinc-700 dark:text-zinc-300 font-bold">
