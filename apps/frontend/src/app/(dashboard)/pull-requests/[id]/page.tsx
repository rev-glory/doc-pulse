...
export default function PullRequestDetailsPage(): React.JSX.Element {
  ...
  if (error || !data) {
    return (
      <div>
        <PageHeader title="Pull Request Details" />
        <ErrorState message={error?.message || 'Failed to retrieve pull request.'} retry={refetch} />
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      <PageHeader
        ...
        actions={
          <div className="flex items-center gap-3">
            <span className={`px-2.5 py-1 rounded-full text-xs font-mono font-bold ${isMerged ? 'bg-purple-100 text-purple-800 dark:bg-purple-950 dark:text-purple-300' : 'bg-emerald-100 text-emerald-800 dark:bg-emerald-950 dark:text-emerald-300'}`}>
              ● {data.status}
            </span>
            {data.url && (
              <a
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-5">
        <MetricCard title="Head Branch" value={data.headBranch} subtitle="Source branch" />
        <MetricCard title="Base Branch" value={data.baseBranch} subtitle="Target destination" />
        <MetricCard title="Trigger Commit SHA" value={data.commitSha.slice(0, 7)} subtitle="Revision" />
        <MetricCard
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        ...
        <div className="lg:col-span-2 space-y-6">
          <SectionCard title="Pull Request Summary" description="Autonomous sync details compiled by DocPulse.">
            <div className="space-y-4">
              <div className="p-4 bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg text-xs leading-relaxed text-zinc-700 dark:text-zinc-300 font-medium">
                {data.body || 'No summary text provided by the documentation compiler node.'}
              </div>
            </div>
          </SectionCard>
        </div>
          ...
          <SectionCard title="SCM & Pipeline Context">
            <div className="space-y-4 text-xs">
              <div className="flex justify-between py-2 border-b border-zinc-100 dark:border-zinc-850">
                <span className="text-zinc-500 font-semibold">Workflow Status</span>
                <WorkflowStatusBadge status={data.workflowStatus} />
              </div>
              <div className="flex justify-between py-2 border-b border-zinc-100 dark:border-zinc-850">
                <span className="text-zinc-500 font-semibold">Created Timestamp</span>
              ...
              {data.mergedAt && (
                <div className="flex justify-between py-2 border-b border-zinc-100 dark:border-zinc-850">
                  <span className="text-zinc-500 font-semibold">Merged Timestamp</span>
                  <span className="font-medium text-zinc-750 dark:text-zinc-350">
                    {new Date(data.mergedAt).toLocaleDateString()} {new Date(data.mergedAt).toLocaleTimeString()}
                  </span>
                </div>
              )}
