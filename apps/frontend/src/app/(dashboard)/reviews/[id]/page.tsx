...
export default function ReviewApprovalWorkspacePage(): React.JSX.Element {
  ...
  if (isLoading) {
    return (
      <div>
        ...
        <LoadingState
          message="Fetching documentation checkpoint snapshot..."
          rows={6}
        />
      </div>
    );
  ...
  const selectedDoc = generatedDocs[selectedDocIndex] as
    | GeneratedDocument
    | undefined;
  ...
  const handleDecision = async (approve: boolean) => {
    ...
    try {
      ...
      setSubmitError(
        err?.message || "Failed to submit review decision. Please try again.",
      );
  ...
  return (
    <div className="space-y-8 animate-fade-in">
        ...
        actions={
          <div className="flex items-center gap-3">
            <span className="text-zinc-500 text-xs font-mono">
              Run ID: #{data.workflowRunId.slice(0, 8)}
            </span>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-5">
        <MetricCard
          title="Repository"
          value={`${data.repositoryOwner}/${data.repositoryName}`}
          subtitle="Target destination"
        />
        <MetricCard
          title="Branch"
          value={data.branch}
          subtitle="Commit branch"
        />
        <MetricCard
          title="Commit SHA"
          value={data.commitSha.slice(0, 7)}
          subtitle="Triggering revision"
        />
        <MetricCard
      ...
      {generatedDocs.length === 0 ? (
        <EmptyState
          ...
          description="This run contains no generated markdown files to review."
        />
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <div className="lg:col-span-1 space-y-4">
            ...
            <div className="flex flex-col gap-1.5">
              {generatedDocs.map((doc, idx) => {
                ...
                const docRev = data.criticReview?.reviews?.find(
                  (r) => r.documentType === doc.type,
                );
                ...
                return (
                  ...
                  >
                    ...
                    <span
                      className={`px-1.5 py-0.5 rounded text-[10px] font-bold ${isApproved ? "bg-emerald-500/10 text-emerald-600" : "bg-rose-500/10 text-rose-600"}`}
                    >
                      {score}/100
                    </span>
                  </button>
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            <SectionCard
              title="Critic Audit"
              description="AI Critic report and review suggestions."
            >
              <div className="space-y-4">
                <div className="p-4 bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg flex items-center justify-between">
                  ...
                  <span
                    className={`text-lg font-black ${documentReview && documentReview.score < 85 ? "text-rose-600" : "text-emerald-600"}`}
                  >
                ...
                {documentReview?.issues && documentReview.issues.length > 0 ? (
                  <div className="space-y-3">
                    ...
                    <div className="space-y-2">
                      {documentReview.issues.map(
                        (issue: CriticIssue, idx: number) => (
                          <div
                            key={idx}
                ...
                {documentReview?.suggestions &&
                  documentReview.suggestions.length > 0 && (
                    <div className="space-y-2 pt-2">
                      <h4 className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider">
                        AI Suggestions
                      </h4>
                      <ul className="list-disc pl-4 text-xs text-zinc-600 dark:text-zinc-400 space-y-1">
                        {documentReview.suggestions.map(
                          (s: string, idx: number) => (
                            <li key={idx}>{s}</li>
                          ),
                        )}
            ...
            <SectionCard
              ...
              description="Submit approval or request updates."
            >
              <div className="space-y-4">
                {submitError && (
