...
import {
  SyncProgressOverlay,
  LogLine,
} from "@/features/repositories/components/sync-progress-overlay";
...
function OnboardingCallbackContent(): React.JSX.Element {
  ...
  const addLog = (text: string, status: LogLine["status"] = "pending") => {
    ...
    setLogs((prev) => {
      ...
      return [
        ...prev,
        { id: Math.random().toString(), text, status, timestamp },
      ];
    });
  };
  ...
  useEffect(() => {
    ...
    const startOnboarding = async () => {
      ...
      if (
        ...
        setErrorState(
          "Invalid onboarding request. No installation ID or setup action found.",
        );
        return;
      }
      ...
      try {
        ...
        while (retryCount < maxRetries) {
          try {
            ...
            verifiedInstallation = installations.find(
        ...
        if (!verifiedInstallation) {
          ...
          setErrorState(
            "Webhook delay timeout. DocPulse has not received the installation webhook from GitHub. Please try again.",
          );
          return;
        }
        ...
        addLog(
          `Successfully verified installation: ${verifiedInstallation.accountLogin} (${verifiedInstallation.accountType})`,
          "success",
        );
        setProgress(25);
        ...
        addLog(
          "Upserting repository nodes into DocPulse workspace database...",
          "pending",
        );
        ...
        const syncPromise =
          RepositoryApi.syncInstallationRepositories(installationId);
        ...
        if (result === null) {
          ...
          addLog(
            "Workspace synchronization deferred to background processing...",
            "running",
          );
          setProgress(85);
        ...
        addLog(
          `Successfully synced ${result.synced} repositories (${result.created} created, ${result.updated} updated)`,
          "success",
        );
        ...
        setErrorState(
          err?.message || "Sync failed due to API connectivity limits.",
        );
      }
    };
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-zinc-950/80 backdrop-blur-md p-4">
        <div className="w-full max-w-md bg-zinc-900 border border-zinc-800 rounded-2xl shadow-2xl p-6 text-center text-zinc-100 animate-fade-in">
          ...
          <h3 className="text-lg font-bold">
            Onboarding Synchronization Error
          </h3>
  ...
  const accountLoginName = logs.find((l) =>
    l.text.includes("Successfully verified installation"),
  )
    ? logs
        .find((l) => l.text.includes("Successfully verified installation"))!
        .text.split(": ")[1]
        ?.split(" (")[0]
    : undefined;
