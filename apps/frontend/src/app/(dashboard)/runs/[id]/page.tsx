  ...
  if (isLoading) {
    return (
      <div>
        ...
        <LoadingState
          message="Connecting to LangGraph agent executor..."
          rows={6}
        />
      </div>
    );
  ...
  if (error || !data) {
    return (
      <div>
        <PageHeader title="Workflow Execution detail" />
        <ErrorState
          message={error?.message || "Workflow run not found."}
          retry={refetch}
        />
      </div>
    );
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      <PageHeader
        ...
        actions={
          <div className="flex items-center gap-3">
            <span
              className={`px-2.5 py-1 rounded-full text-xs font-mono font-bold ${isConnected ? "bg-emerald-100 text-emerald-800 dark:bg-emerald-950 dark:text-emerald-300" : "bg-zinc-200 text-zinc-600"}`}
            >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-5">
        <MetricCard
          title="Commit SHA"
          value={data.commitSha?.slice(0, 7) || "HEAD"}
          subtitle={data.branch}
        />
        <MetricCard
          title="Duration"
          value={`${durationSec}${typeof durationSec === "string" && durationSec.endsWith("s") ? "" : "s"}`}
          subtitle={
            data.startedAt
        ...
        />
        <MetricCard
          title="Active LangGraph Node"
          value={data.currentNode || "Queued"}
          status="success"
        />
        <MetricCard
      ...
      <SectionCard
        ...
        description="Real-time WebSocket event feed from worker container."
      >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
        ...
        <SectionCard
          title="Node History & Agent Checkpoints"
          description="Dynamic execution logs mapped from LangGraph checkpoint state."
        >
        ...
        <div className="space-y-6">
          <SectionCard
            title="Checkpoint Snapshot Information"
            description="Serialized documentation inventory, rating profiles, and SCM outputs."
          >
            <div className="space-y-5 text-xs">
              {/* Generated Docs summary */}
              <div className="space-y-2">
                <h4 className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider">
                  Generated Documentation ({generatedDocs.length})
                </h4>
                  ...
                  <div className="space-y-1.5 max-h-[140px] overflow-y-auto">
                    {generatedDocs.map((doc) => (
                      ...
                      >
                        ...
                        <span className="font-mono text-[10px] text-zinc-450 bg-zinc-100 dark:bg-zinc-800 px-1 rounded">
                          {doc.type}
                        </span>
                      </div>
                    ))}
              ...
              <div className="space-y-2 pt-2 border-t border-zinc-100 dark:border-zinc-850">
                ...
                {criticReview ? (
                  <div className="flex items-center justify-between p-3 bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-850 rounded-lg">
                    <div>
                      <span className="font-bold text-zinc-800 dark:text-zinc-250 block">
                        AI Critic score
                      </span>
                      <span className="text-[10px] text-zinc-400">
                        {criticReview.approvedCount} Approved •{" "}
                        {criticReview.failedCount} Refused
                      </span>
                    </div>
                    <span
                      className={`text-lg font-black ${criticReview.passed ? "text-emerald-600" : "text-rose-600"}`}
                    >
              ...
              {data.pullRequestUrl &&
                data.gitOperationStatus !== "NoPullRequestRequired" && (
                  <div className="pt-2 border-t border-zinc-100 dark:border-zinc-850">
                    <a
          ...
          <SectionCard title="BullMQ Execution Queue">
            <QueueStatus
              status={queueStatus || currentStatus}
            ...
            {data.completionReason === "SKIPPED" && (
              <div className="mt-4 p-3.5 bg-orange-50 border border-orange-200 dark:bg-orange-950/20 dark:border-orange-900/40 text-orange-850 dark:text-orange-300 rounded-lg text-xs leading-relaxed">
                ...
                {data.skipReason || "Documentation-only changes detected."}
              </div>
            )}
            {data.errorMessage && (
