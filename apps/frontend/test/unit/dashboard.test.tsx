...
describe('Frontend Dashboard Presentation & Components Suite', () => {
  test('WorkflowStatusBadge renders correct label and style mapping', () => {
    const runMarkup = renderToStaticMarkup(<WorkflowStatusBadge status="running" />);
    assert.ok(runMarkup.includes('Running'), 'Should render Running label');
    assert.ok(runMarkup.includes('blue'), 'Should map running status to blue color');

    const doneMarkup = renderToStaticMarkup(<WorkflowStatusBadge status="completed" />);
    assert.ok(doneMarkup.includes('Completed'), 'Should render Completed label');
    assert.ok(doneMarkup.includes('green'), 'Should map completed status to green color');
  });

  test('WorkflowTimeline renders stage progression indicators', () => {
    const markup = renderToStaticMarkup(
      <WorkflowTimeline currentStage={RealtimeWorkflowStage.Writing} status="running" />,
    );
    assert.ok(markup.includes('Queued'), 'Should render Queued step');
    assert.ok(markup.includes('[x]'), 'Should mark past steps as done');
    assert.ok(markup.includes('[&gt;]'), 'Should mark current step with active indicator');
  });

  test('LiveProgress bounds progress percentages and displays error messages', () => {
    const markup = renderToStaticMarkup(
      <LiveProgress stage="Analyzing" progress={120} status="failed" errorMessage="Syntax error in AST" />,
    );
    assert.ok(markup.includes('100%'), 'Should bound percentage to max 100%');
    assert.ok(markup.includes('Syntax error in AST'), 'Should render telemetry error message');
  });
  ...
  test('RepositoryTable renders empty state when list is empty', () => {
    ...
    assert.ok(markup.includes('No repositories connected'), 'Should display empty state title');
  });
  ...
  test('RepositoryTable renders connected repository rows', () => {
    ...
    const markup = renderToStaticMarkup(<RepositoryTable repositories={mockRepos} />);
  ...
  test('WorkflowRunsTable renders execution rows with progress bar', () => {
    ...
    assert.ok(markup.includes('80%'), 'Should render bounded progress percentage');
  });
  ...
  test('Mock API & WebSocket state simulation verification', () => {
    ...
    assert.strictEqual(mockWsPayload.data.stage, RealtimeWorkflowStage.CreatingPR);
  });
});
