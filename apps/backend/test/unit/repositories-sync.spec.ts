...
describe("RepositoriesService - Synchronization & Reconciliation", () => {
  ...
  beforeEach(() => {
    mockRepositoriesPersistence = {
      syncUpsert: mock.fn(async (data: any) => ({
        id: `repo-${data.githubRepositoryId}`,
        ...data,
      })),
    };
    ...
    mockInstallationsPersistence = {
      findByInstallationId: mock.fn(async (id: number) =>
        id === 99999 ? mockInstallation : null,
      ),
  ...
  it("repository added: should insert new repositories fetched from GitHub", async () => {
    mockGitHubRepositoryService.listInstallationRepositories.mock.mockImplementation(
      async () => [
        {
  ...
  it("repository removed: should mark disconnected repositories inactive", async () => {
    ...
    const updateCall =
      mockPrisma.repository.updateMany.mock.calls[0].arguments[0];
  ...
  it("reactivation regression: install -> remove -> re-add reactivates existing database row without duplication", async () => {
    ...
    mockPrisma.repository.findMany.mock.mockImplementationOnce(async () => []);
    const resInstall = await service.syncInstallationRepositories(
      99999,
      mockUser,
    );
    ...
    const resRemove = await service.syncInstallationRepositories(
      99999,
      mockUser,
    );
    ...
    const resReadd = await service.syncInstallationRepositories(
      99999,
      mockUser,
    );
