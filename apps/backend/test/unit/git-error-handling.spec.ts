...
import {
  GitException,
  isGitException,
} from "../../src/modules/git-operations/errors/git-exception";
...
function makeGitException(
  ...
  return new GitException(
    code,
    message,
    { provider: "SimpleGit", repository: "repo/test" },
    "testOp",
  );
}
...
describe("GitErrorClassifier", () => {
  it("marks NETWORK_ERROR as retryable", () => {
    assert.equal(
      GitErrorClassifier.isRetryable(GitErrorCode.NETWORK_ERROR),
      true,
    );
  });
  ...
  it("marks RATE_LIMITED as retryable", () => {
    assert.equal(
      GitErrorClassifier.isRetryable(GitErrorCode.RATE_LIMITED),
      true,
    );
  });
  ...
  it("marks REPOSITORY_LOCKED as retryable", () => {
    assert.equal(
      GitErrorClassifier.isRetryable(GitErrorCode.REPOSITORY_LOCKED),
      true,
    );
  });
  ...
  it("marks AUTHENTICATION_FAILED as permanent", () => {
    assert.equal(
      GitErrorClassifier.isPermanent(GitErrorCode.AUTHENTICATION_FAILED),
      true,
    );
  });
  ...
  it("marks REPOSITORY_NOT_FOUND as permanent", () => {
    assert.equal(
      GitErrorClassifier.isPermanent(GitErrorCode.REPOSITORY_NOT_FOUND),
      true,
    );
  });
  ...
  it("marks MERGE_CONFLICT as permanent", () => {
    assert.equal(
      GitErrorClassifier.isPermanent(GitErrorCode.MERGE_CONFLICT),
      true,
    );
  });
  ...
  it("marks PUSH_REJECTED as permanent", () => {
    assert.equal(
      GitErrorClassifier.isPermanent(GitErrorCode.PUSH_REJECTED),
      true,
    );
  });
...
describe("GitErrorSanitizer", () => {
  it("masks embedded username:password credentials in URL", () => {
    const input =
      "fatal: repository https://user:secret-password@github.com/org/repo.git not found";
  ...
  it("masks x-access-token style OAuth URL", () => {
    const input =
      "fatal: https://x-access-token:ghp_abc123def456ghi789jkl012mno345pqr678@github.com/org/repo.git";
    const out = GitErrorSanitizer.sanitize(input);
    assert.ok(
      !out.includes("ghp_abc123def456ghi789jkl012mno345pqr678"),
  ...
  it("masks bearer authorization headers", () => {
    ...
    const out = GitErrorSanitizer.sanitize(input);
    assert.ok(
      !out.includes("eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9"),
      "token should be masked",
    );
    assert.ok(out.includes("***TOKEN***"), "should contain masked placeholder");
  });
...
describe("GitException", () => {
  it("constructs with the correct code and sanitized message", () => {
    ...
    assert.ok(
      !ex.message.includes(token),
      "raw token must not appear in message",
    );
    assert.ok(
      ex.message.includes("***TOKEN***"),
      "sanitized placeholder must appear",
    );
...
describe("SimpleGitErrorMapper", () => {
  ...
  it('maps "authentication failed" -> AUTHENTICATION_FAILED', () => {
    assert.equal(
      map("remote: authentication failed").code,
      GitErrorCode.AUTHENTICATION_FAILED,
    );
  });
  ...
  it('maps "could not read username" -> AUTHENTICATION_FAILED', () => {
    assert.equal(
      map("fatal: could not read username").code,
      GitErrorCode.AUTHENTICATION_FAILED,
    );
  });
  ...
  it('maps "permission denied" -> AUTHENTICATION_FAILED', () => {
    assert.equal(
      map("Permission denied (publickey)").code,
      GitErrorCode.AUTHENTICATION_FAILED,
    );
  });
  ...
  it('maps "repository not found" -> REPOSITORY_NOT_FOUND', () => {
    assert.equal(
      map("ERROR: Repository not found.").code,
  ...
  it('maps "could not read from remote repository" -> REPOSITORY_NOT_FOUND', () => {
    assert.equal(
      map("fatal: Could not read from remote repository").code,
      GitErrorCode.REPOSITORY_NOT_FOUND,
    );
  });
  ...
  it('maps "already exists" -> BRANCH_ALREADY_EXISTS', () => {
    assert.equal(
      map("fatal: A branch named 'main' already exists.").code,
      GitErrorCode.BRANCH_ALREADY_EXISTS,
    );
  });

  it('maps "merge conflict" -> MERGE_CONFLICT', () => {
    assert.equal(
      map("error: merge conflict detected").code,
  ...
  it('maps "updates were rejected" -> PUSH_REJECTED', () => {
    assert.equal(
      map("error: failed to push some refs; updates were rejected").code,
  ...
  it('maps "protected branch" -> PROTECTED_BRANCH', () => {
    assert.equal(
      map("error: GH006: Protected branch update failed").code,
      GitErrorCode.PROTECTED_BRANCH,
    );
  });

  it('maps "could not resolve host" -> NETWORK_ERROR', () => {
    assert.equal(
      map("fatal: unable to access: Could not resolve host: github.com").code,
  ...
  it('maps "timeout" -> TIMEOUT', () => {
    assert.equal(
      map("error: git operation timed out").code,
  ...
  it('maps "index.lock" -> REPOSITORY_LOCKED', () => {
    assert.equal(
      map("fatal: Unable to create .git/index.lock: File exists").code,
  ...
  it("maps unrecognized error -> UNKNOWN", () => {
    assert.equal(
      map("some completely unknown error XYZ").code,
  ...
  it("sanitizes embedded tokens from mapped exception messages", () => {
    ...
    const ex = mapper.mapError({
      operation: "clone",
      error: {
        message: `fatal: https://x-access-token:${token}@github.com/org/repo.git not found`,
      },
    });
    assert.ok(
      !ex.message.includes(token),
      "token must be stripped from mapped message",
    );
  });
});
...
describe("classifyWorkflowError — nested cause chain traversal", () => {
  ...
  it("classifies WorkflowExecutionException -> CloneFailedException -> GitException[NETWORK] as TRANSIENT", () => {
    const gitEx = makeGitException(GitErrorCode.NETWORK_ERROR);
    const cloneEx = new CloneFailedException("Clone failed", gitEx);
    const workflowEx = new WorkflowExecutionException(
      "Workflow failed",
      cloneEx,
    );
    assert.equal(
      classifyWorkflowError(workflowEx),
      QueueErrorClassification.TRANSIENT,
    );
  });
  ...
  it("does not enter infinite loops on circular cause references", () => {
    ...
    assert.ok(
      result === QueueErrorClassification.TRANSIENT ||
        result === QueueErrorClassification.PERMANENT,
    );
  });
});
