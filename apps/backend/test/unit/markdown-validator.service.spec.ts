...
describe('MarkdownValidatorService Unit Tests', () => {
  ...
  it('should flag errors on missing headings or unclosed code fences', () => {
    ...
    assert.ok(res.errors.some((e) => e.message.includes('no Markdown headings')));
    assert.ok(res.errors.some((e) => e.message.includes('Unclosed Markdown code fence')));
  });
  ...
  it('should flag diagnostic warnings on duplicate headings and empty sections', () => {
    ...
    assert.ok(res.warnings.some((w) => w.message.includes('Duplicate level 2 heading')));
    assert.ok(res.warnings.some((w) => w.message.includes('contains an empty section')));
  });
});
