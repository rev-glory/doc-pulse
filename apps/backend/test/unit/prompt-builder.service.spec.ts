...
describe('PromptBuilderService Unit Tests', () => {
  ...
  it('should correctly format and include AI and human review feedback on subsequent iterations', async () => {
    ...
    assert.ok(compiled.userPrompt.includes('This documentation has been regenerated 2 time(s)'));
    ...
    assert.ok(compiled.userPrompt.includes('MAJOR] Content: Missing build instructions'));
    assert.ok(!compiled.userPrompt.includes('MINOR] Schema: Type formatting issues')); // should be filtered out
    assert.ok(compiled.userPrompt.includes('Suggestions'));
    assert.ok(compiled.userPrompt.includes('Please add dynamic quickstart steps'));
    assert.ok(!compiled.userPrompt.includes('Please add authentication example')); // should be filtered out
