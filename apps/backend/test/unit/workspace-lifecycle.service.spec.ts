...
describe('WorkspaceLifecycleService', () => {
  ...
  it('should check if workspace exists', async () => {
    mockWorkspaceService.repositoryExists.mock.mockImplementation(async (repoId: string) => {
  ...
  it('should get workspace path', () => {
    mockWorkspaceService.getWorkspacePath.mock.mockImplementation((repoId: string) => {
      assert.equal(repoId, 'repo-123');
      return '/tmp/repo-123';
    });
  ...
  it('should delete workspace', async () => {
    mockLockService.acquireLock.mock.mockImplementation(async (repoId: string) => {
      assert.equal(repoId, 'repo-123');
      return () => {};
    });
    mockWorkspaceService.removeRepository.mock.mockImplementation(async (repoId: string) => {
      assert.equal(repoId, 'repo-123');
    });
  ...
  describe('shouldCleanup', () => {
    it('should return true when there are no active workflow runs', async () => {
      mockPrisma.workflowRun.findMany.mock.mockImplementation(async (query: any) => {
        assert.deepEqual(query.where.status.in, ['QUEUED', 'RUNNING', 'CHECKPOINTED', 'WAITING_FOR_REVIEW']);
        return [];
      });
  ...
  describe('cleanupExpiredWorkspaces', () => {
    it('should delete disconnected repository workspace', async () => {
      mockWorkspaceService.getClonedRepositoryIds.mock.mockImplementation(async () => ['repo-disconnected']);
      mockPrisma.repository.findUnique.mock.mockImplementation(async () => null);

      await service.cleanupExpiredWorkspaces();

      assert.equal(mockWorkspaceService.removeRepository.mock.calls.length, 1);
      assert.equal(mockWorkspaceService.removeRepository.mock.calls[0].arguments[0], 'repo-disconnected');
    });

    it('should delete workspace if there are no workflow runs', async () => {
      mockWorkspaceService.getClonedRepositoryIds.mock.mockImplementation(async () => ['repo-no-runs']);
      mockPrisma.repository.findUnique.mock.mockImplementation(async () => ({ id: 'repo-no-runs' }));
      mockPrisma.workflowRun.findMany.mock.mockImplementation(async () => []);

      await service.cleanupExpiredWorkspaces();

      assert.equal(mockWorkspaceService.removeRepository.mock.calls.length, 1);
      assert.equal(mockWorkspaceService.removeRepository.mock.calls[0].arguments[0], 'repo-no-runs');
    });

    it('should delete workspace if all workflows are completed and expired', async () => {
      mockWorkspaceService.getClonedRepositoryIds.mock.mockImplementation(async () => ['repo-expired']);
      mockPrisma.repository.findUnique.mock.mockImplementation(async () => ({ id: 'repo-expired' }));
      ...
      assert.equal(mockWorkspaceService.removeRepository.mock.calls.length, 1);
      assert.equal(mockWorkspaceService.removeRepository.mock.calls[0].arguments[0], 'repo-expired');
    });

    it('should not delete workspace if all workflows are completed but NOT expired', async () => {
      mockWorkspaceService.getClonedRepositoryIds.mock.mockImplementation(async () => ['repo-not-expired']);
      mockPrisma.repository.findUnique.mock.mockImplementation(async () => ({ id: 'repo-not-expired' }));
    ...
    it('should delete workspace if active run is expired (abandoned)', async () => {
      mockWorkspaceService.getClonedRepositoryIds.mock.mockImplementation(async () => ['repo-abandoned']);
      mockPrisma.repository.findUnique.mock.mockImplementation(async () => ({ id: 'repo-abandoned' }));
      ...
      assert.equal(mockWorkspaceService.removeRepository.mock.calls.length, 1);
      assert.equal(mockWorkspaceService.removeRepository.mock.calls[0].arguments[0], 'repo-abandoned');
    });
  });
});
