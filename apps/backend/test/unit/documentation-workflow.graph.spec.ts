...
describe("Documentation Workflow Graph Routing Unit Tests", () => {
  const mockAdapters = {
    ...
    repositoryAnalyzerStep: async (state: any) => ({
      ...state,
      currentNode: WorkflowNodeName.RepositoryAnalyzer,
    }),
    documentationLocatorStep: async (state: any) => ({
      ...state,
      currentNode: WorkflowNodeName.DocumentationLocator,
    }),
    codebaseAnalyzerStep: async (state: any) => ({
      ...state,
      currentNode: WorkflowNodeName.CodebaseAnalyzer,
    }),
    technicalWriterStep: async (state: any) => {
      ...
      const nextIteration = isRegeneration
        ? currentIteration + 1
        : currentIteration;
      return {
        ...state,
    ...
    gitCommitStep: async (state: any) => ({
      ...state,
      currentNode: WorkflowNodeName.GitCommit,
    }),
    pushBranchStep: async (state: any) => ({
      ...state,
      currentNode: WorkflowNodeName.PushBranch,
    }),
    createPullRequestStep: async (state: any) => ({
      ...state,
      currentNode: WorkflowNodeName.CreatePullRequest,
    }),
  } as any;
