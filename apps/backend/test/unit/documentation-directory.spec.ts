...
import { isValidDocumentationDirectory, normalizeDocumentationDirectory } from '../../src/modules/repositories/validators/documentation-directory.validator';
...
describe('Configurable Docs Directory - Validator & Normalizer', () => {
  it('should normalize documentation directories correctly', () => {
    ...
    assert.equal(normalizeDocumentationDirectory('docs///'), 'docs');
    assert.equal(normalizeDocumentationDirectory('project//docs/'), 'project/docs');
    assert.equal(normalizeDocumentationDirectory('\\project\\\\docs\\\\\\'), 'project/docs');
  });
...
describe('Configurable Docs Directory - Service Layer', () => {
  it('should validate and normalize database values on PATCH', async () => {
    ...
    const res1 = await service.updateRepository('repo-1', { documentationDirectory: 'my///docs///' }, { id: 'user-1' } as any);
    assert.equal(res1.documentationDirectory, 'my/docs');

    // 2. Traversal path update should throw
    await assert.rejects(
      () => service.updateRepository('repo-1', { documentationDirectory: '../invalid' }, { id: 'user-1' } as any),
      (err: any) => err.message.includes('Invalid documentation directory'),
    );

    // 3. Absolute path update should throw
    await assert.rejects(
      () => service.updateRepository('repo-1', { documentationDirectory: '/absolute' }, { id: 'user-1' } as any),
      (err: any) => err.message.includes('Invalid documentation directory'),
    );
  });
});
...
describe('Configurable Docs Directory - Document Writer & Loader', () => {
  ...
  it('should write documents to repository root (cleanDir === ".")', async () => {
    ...
    const readmeContent = await fs.readFile(path.join(tempWorkspace, 'README.md'), 'utf8');
  ...
  it('should write documents inside a nested docs/ directory recursively', async () => {
    ...
    const res = await writer.writeDocuments(tempWorkspace, 'run-1', docs, 'project/docs///');
    assert.deepEqual(res.writtenFiles, [path.join('project/docs', 'README.md')]);

    const readmeContent = await fs.readFile(path.join(tempWorkspace, 'project/docs/README.md'), 'utf8');
    assert.equal(readmeContent, '# Title');
  });

  it('should return empty documentation inventory instead of throwing when directory does not exist', async () => {
    const inv = await buildDocumentationInventory(tempWorkspace, 'non-existent-dir');
  ...
  it('should read documentation from configured directory and ignore other places', async () => {
    ...
    await fs.mkdir(path.join(tempWorkspace, 'project/docs'), { recursive: true });

    await fs.writeFile(path.join(tempWorkspace, 'docs/README.md'), '# Old Readme');
    await fs.writeFile(path.join(tempWorkspace, 'project/docs/README.md'), '# New Readme');
    ...
    const invProject = await buildDocumentationInventory(tempWorkspace, 'project/docs');
    assert.equal(invProject.documentationFiles.length, 1);
    assert.equal(invProject.documentationFiles[0].fileName, 'README.md');
    assert.equal(invProject.documentationFiles[0].path, 'project/docs/README.md');
  });
});
