...
export class WorkflowGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  public handleSubscribe(
    ...
    @MessageBody() data: { workflowId?: string; repositoryId?: string; runId?: string },
  ): void {
    const { workflowId, repositoryId, runId } = data || {};
    if (workflowId) {
      void client.join(`workflow:${workflowId}`);
      this.logger.debug(`Client [${client.id}] subscribed to room [workflow:${workflowId}]`);
    }
    if (repositoryId) {
      void client.join(`repository:${repositoryId}`);
      this.logger.debug(`Client [${client.id}] subscribed to room [repository:${repositoryId}]`);
    }
    if (runId) {
      void client.join(`run:${runId}`);
      this.logger.debug(`Client [${client.id}] subscribed to room [run:${runId}]`);
    }
  }
  ...
  public handleUnsubscribe(
    ...
    if (workflowId) {
      ...
      this.logger.debug(`Client [${client.id}] unsubscribed from room [workflow:${workflowId}]`);
    }
    if (repositoryId) {
      void client.leave(`repository:${repositoryId}`);
      this.logger.debug(`Client [${client.id}] unsubscribed from room [repository:${repositoryId}]`);
    }
    if (runId) {
      void client.leave(`run:${runId}`);
      this.logger.debug(`Client [${client.id}] unsubscribed from room [run:${runId}]`);
    }
  }
  ...
  public emitEvent(payload: RealtimeEventPayload): void {
    if (!this.server) {
      this.logger.warn('WebSocket server is not initialized yet. Skipping emit.');
      return;
    }
    ...
    if (payload.repositoryId) {
      this.server.to(`repository:${payload.repositoryId}`).emit(eventName, payload);
    }
