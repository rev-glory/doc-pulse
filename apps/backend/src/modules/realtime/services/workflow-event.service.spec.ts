...
describe("WorkflowEventService", () => {
  ...
  it("should publish general workflow event correctly", () => {
    ...
    assert.strictEqual(
      emittedPayloads[0].stage,
      RealtimeWorkflowStage.Analyzing,
    );
  ...
  it("should map node name to correct realtime stage", () => {
    assert.strictEqual(
      service.mapNodeToStage("RepositoryAnalyzer"),
      RealtimeWorkflowStage.Analyzing,
    );
    assert.strictEqual(
      service.mapNodeToStage("TechnicalWriter"),
      RealtimeWorkflowStage.Writing,
    );
    assert.strictEqual(
      service.mapNodeToStage("DocumentationCritic"),
      RealtimeWorkflowStage.Reviewing,
    );
    assert.strictEqual(
      service.mapNodeToStage("GitCommit"),
