...
import { SourceCodeAnalysis, ArchitectureAnalysis, ImportantFile } from '../../../../domain/source-code-analysis/source-code-analysis';
  ...
  public async analyze(index: RepositoryIndex): Promise<Partial<SourceCodeAnalysis>> {
    ...
    for (const file of index.files) {
      ...
      if (importantFileNames[name as keyof typeof importantFileNames]) {
        if (importantFiles.length < 100 && !importantFiles.some((f) => f.path === file.relativePath)) {
          importantFiles.push({
            path: file.relativePath,
            reason: importantFileNames[name as keyof typeof importantFileNames]!,
          });
        }
      }
    ...
    if (dirsStr.includes('src/modules')) {
      ...
      patterns.push('Dependency Injection', 'Repository Pattern', 'Decorator-driven Routing');
      layers.push('Controllers (REST API)', 'Services (Business Logic)', 'Modules (Context Boundaries)', 'Database Layer (ORMs)');
    } else if (dirsStr.includes('controllers') && dirsStr.includes('services')) {
      styles.push('Controller-Service-Repository Pattern');
      layers.push('Controller Router Layer', 'Service Business Logic Layer');
    } else if (dirsStr.includes('src/components') || dirsStr.includes('src/pages')) {
    ...
    if (dirsStr.includes('middleware') || dirsStr.includes('guards') || dirsStr.includes('interceptors')) {
    ...
    const architecture: ArchitectureAnalysis = {
      ...
      patterns,
      layers,
      moduleStructure: importantDirectories.filter((d) => d.includes('module') || d.split('/').length === 1),
    };
    ...
    return {
      ...
      modules: importantDirectories.filter((d) => d.startsWith('src/modules') || (d.includes('modules') && d.split('/').length === 2)).slice(0, 50),
    };
  }
}
