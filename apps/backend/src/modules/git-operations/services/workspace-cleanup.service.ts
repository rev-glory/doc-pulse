...
export class WorkspaceCleanupService {
  ...
  async cleanupRepository(repositoryId: string): Promise<void> {
    this.logger.log(
      `Repository access revoked. Cleaning workspace for repository: ${repositoryId}`,
    );
    ...
    try {
      ...
      this.logger.error(
        `Failed to delete workspace directory for repository ${repositoryId}: ${error.message}`,
      );
    }
    ...
    try {
      ...
      this.logger.error(
        `Failed to remove repository metadata from database for repository ${repositoryId}: ${error.message}`,
      );
    }
