...
function compileDocumentationGraph(
  ...
  const workflow = new StateGraph(WorkflowGraphAnnotation)
    .addNode(WorkflowNodeName.EarlySkip, (state: WorkflowGraphState) =>
      adapters.earlySkipStep(state),
    )
    .addNode(WorkflowNodeName.RepositoryAnalyzer, (state: WorkflowGraphState) =>
      adapters.repositoryAnalyzerStep(state),
    )
    .addNode(
      WorkflowNodeName.DocumentationLocator,
      (state: WorkflowGraphState) => adapters.documentationLocatorStep(state),
    )
    .addNode(WorkflowNodeName.CodebaseAnalyzer, (state: WorkflowGraphState) =>
      adapters.codebaseAnalyzerStep(state),
    )
    .addNode(WorkflowNodeName.TechnicalWriter, (state: WorkflowGraphState) =>
      adapters.technicalWriterStep(state),
    )
    .addNode(
      WorkflowNodeName.DocumentationCritic,
      (state: WorkflowGraphState) => adapters.documentationCriticStep(state),
    )
    .addNode(WorkflowNodeName.GitCommit, (state: WorkflowGraphState) =>
      adapters.gitCommitStep(state),
    )
    .addNode(WorkflowNodeName.PushBranch, (state: WorkflowGraphState) =>
      adapters.pushBranchStep(state),
    )
    .addNode(WorkflowNodeName.CreatePullRequest, (state: WorkflowGraphState) =>
      adapters.createPullRequestStep(state),
    )
...
export type CompiledDocumentationGraph = ReturnType<
  typeof compileDocumentationGraph
>;
