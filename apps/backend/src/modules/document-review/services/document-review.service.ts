...
export class DocumentReviewService {
  ...
  public async reviewDocuments(
    ...
    const repoIdentifier =
      ...
      "unknown-repository";
    this.logger.log(
      `[${runId}] Reviewing ${generatedDocuments?.length || 0} documents for repository: ${repoIdentifier}`,
    );

    if (!repository || !repository.name) {
      throw new BadRequestException(
        "Invalid repository summary provided to DocumentReviewService",
      );
    }
    if (!documentation) {
      throw new BadRequestException(
        "Missing documentation inventory in DocumentReviewService",
      );
    }
    if (!generatedDocuments || generatedDocuments.length === 0) {
      throw new BadRequestException(
        "No generated documents provided to review",
      );
    }

    // 1. Build deterministic repository context
    const repoContext = this.contextBuilder.buildContext({
      repository,
      documentation,
    } as any);
    ...
    const compiledPrompt = await this.promptBuilder.buildBatchCriticPrompt(
      generatedDocuments,
      repoContext,
    );
    ...
    const completedReviews: DocumentationReview[] = generatedDocuments.map(
      (doc) => {
        ...
        const perDocMetrics = {
          ...
          promptTokens: Math.round(
            usage.promptTokens / generatedDocuments.length,
          ),
          completionTokens: Math.round(
            usage.completionTokens / generatedDocuments.length,
          ),
          totalTokens: Math.round(
            usage.totalTokens / generatedDocuments.length,
          ),
        ...
        if (!rawOutput) {
          return {
            ...
            suggestions: [
              "Regenerate document due to review subsystem exception",
            ],
        ...
        const reviewResult = this.reviewEvaluator.evaluate(
          rawOutput,
          validationResult,
          doc.type,
          perDocMetrics,
