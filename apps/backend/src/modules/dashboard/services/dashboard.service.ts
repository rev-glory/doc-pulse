  ...
  async getDashboardStats(user: User): Promise<DashboardStats> {
    ...
    const [
      ...
      this.prisma.workflowRun.count({
        where: { repository: { ownerId: user.id }, status: "RUNNING" },
      }),
      this.prisma.workflowRun.count({
        where: { repository: { ownerId: user.id }, status: "COMPLETED" },
      }),
      this.prisma.workflowRun.count({
        where: { repository: { ownerId: user.id }, status: "FAILED" },
      }),
    ...
    return {
      ...
      queueStatus: {
        ...
        status:
          activeWorkflows > 0 ? "active" : queuedCount > 0 ? "waiting" : "idle",
      },
    ...
    return {
      ...
      workflow: {
        ...
        webhookUrl:
          ...
          "http://localhost:3001/github/webhooks",
